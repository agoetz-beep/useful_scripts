...
v_boson_pt_hist.Sumw2() #Bin-Fehler aktivieren
...
for filename in filenames:
    ...
    for event in events:
        count += 1
        if count % 1000 == 0: #wenn dr Rest von count/1000 gleich Null ist, also ein Vielfaches von 1000
            print (count)
        ...
        if boson == "Zvv":
            for p in pruned:
                if p.isPromptFinalState() and (abs(p.pdgId()) == 12 or abs(p.pdgId()) == 14 or abs(p.pdgId()) == 16): #Abfrage ob Neutrino/Anti-Neutrino
        ...
        if boson == "Zvv":
            ...
            if decay_prods[0].pdgId() + decay_prods[1].pdgId() != 0: #ist ungleich, skip the rest of the code inside a loop for the current iteration
                continue
            ...
            if decay_prods[0].pdgId() * decay_prods[1].pdgId() >= 0 or abs(abs(decay_prods[0].pdgId()) - abs(decay_prods[1].pdgId())) != 1: 
                #Neutrino und zugehöriges Lepton liegen genau eins auseinander
                continue
        ...
        # one has to find isolated photons using the isolation prescription in https://arxiv.org/pdf/1705.04664.pdf
            ...
            for photon in photons:
                isolated = True
                R_dyn = 91.1876 / (photon.pt() * sqrt(epsilon_0_dyn)) #Definition aus paper S.6
